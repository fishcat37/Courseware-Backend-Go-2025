...
2. Kafka 4.x 用 KRaft 管理元数据，将消息存储在对应分区中，每个消息都会分配一个唯一的偏移量（Offset）
3. 消费者（Go 代码）从 Topic 拉取消息并处理，同时记录已消费的偏移量位置
4. 消费者可以通过重置偏移量来重新消费消息，或从指定位置开始消费
...
用记事本打开 `..\..\config\server.properties` 文件，找到 `log.dirs=/tmp/kraft-combined-logs` 这一行，修改为：
   ```
   log.dirs=../data/kraft-combined-logs
   ```
   保存文件。

3. 执行初始化命令（生成集群 ID，初始化元数据存储）：
   ```
   .\kafka-storage.bat format --cluster-id abc123 --config ..\..\config\server.properties --standalone --ignore-formatted
   ```

4. 初始化成功标志：终端无报错，且 BIN目录下会生成 data/kraft-combined-logs 文件夹（用于存储元数据和消息日志），无需手动创建。
...
使用 Docker 部署 Kafka 4.x 是最便捷的方式，无需安装 Java 环境，一键启动，适合快速开发和测试。

#### 前置条件

- 已安装 Docker Desktop（Windows/Mac）或 Docker Engine（Linux）
- Docker 版本 19.03+（推荐最新版本）

#### 步骤 1：拉取 Kafka 4.x 镜像

打开终端（Windows 上使用 PowerShell 或 cmd），执行以下命令拉取官方 Kafka 4.x 镜像：
...
为了让 Kafka 容器能够正常通信，创建一个专用网络：

```bash
docker network create kafka-network
```

#### 步骤 3：启动 Kafka 4.x 容器

使用以下命令启动 Kafka 4.x 容器，自动配置 KRaft 模式：
...
执行以下命令检查 Kafka 容器是否正常运行：

```bash
docker ps
```

若看到 STATUS 为 "Up"，说明 Kafka 容器已成功启动。

#### 步骤 5：创建 Topic（与传统方式相同）

在 Docker 容器中执行命令创建 Topic：

```bash
docker exec -it kafka kafka-topics.sh --create --bootstrap-server localhost:9092 --topic go-test-topic --partitions 1 --replication-factor 1
```

创建成功后，会提示 "Created topic go-test-topic."

#### 步骤 6：验证 Topic 是否创建成功
...
- 端口冲突：如果 9092 端口已被占用，可修改 `-p` 参数中的主机端口，例如 `-p 9093:9092`
- 容器启动失败：检查 Docker 网络是否创建成功，或尝试重启 Docker 服务
- 连接超时：确保 Docker 容器在运行状态，且防火墙未阻止 9092 端口

#### 停止和删除 Kafka 容器

当不需要 Kafka 服务时，可执行以下命令停止并删除容器：
